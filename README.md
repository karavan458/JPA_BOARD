...
### 세션 인증, JPA 연관관계 매핑, QueryDSL 기반 동적 쿼리, N+1 문제 해결까지 신입 백엔드 개발자로서의 핵심 역량을 직접 부딪히며 구축한 프로젝트입니다.
...
* **Architectural Insight**
  * **도메인 독립성(Domain Integrity)**
    * **통찰** : 성능 최적화를 위해 `PostRepostiroy`에서 댓글을 조인하여 집계도 가능하나, 이는 `Post`가 `Comment`의 내부를 알아야하는 강한 결합을 초래.
